...
class Connection
{
        ...
        while (true) {
            ...
            switch ($marker) {
                ...
                case Constants::PACKET_AUTH_MORE_DATA:
                    ...
                    throw new Exception('Unexpected auth status: ' . $status);
                default:
                    throw new Exception('Unexpected auth packet marker: ' . $marker);
            }
        }
    }
    ...
    private function fullAuth(string $nonce): void
    {
        ...
        if (\ord($packet[0]) !== Constants::PACKET_AUTH_MORE_DATA) {
            throw new Exception('Expected public key in auth exchange');
        }
    ...
    private function throwError(string $packet): never
    {
        ...
        throw new Exception("MySQL error {$code}: {$message}");
    }
}
